...
if dupont_data and dupont_data.get('full_data'):
    ...
    print(f"最新数据日期: {latest_data.get('date', latest_data.get('报告期', '未知'))}")
    print(f"净资产收益率: {latest_data.get('净资产收益率(%)', latest_data.get('净资产收益率', '未知'))}%")
    print(f"经营利润率: {latest_data.get('经营利润率(%)', latest_data.get('经营利润率', '未知'))}")
    print(f"利息负担: {latest_data.get('利息负担(%)', latest_data.get('考虑利息负担', '未知'))}")
    # 打印更多可用的利润相关信息
    print(f"\n可用利润相关字段:")
    print(f"营业利润率: {latest_data.get('营业利润率', '未知')}")
    print(f"毛利率: {latest_data.get('毛利率', '未知')}")
    print(f"销售净利率: {latest_data.get('销售净利率', '未知')}")
else:
    print("获取杜邦分析数据失败")
